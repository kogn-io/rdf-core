...
| `rdf-cid` | `io.kogn.rdf:rdf-cid` | Content-addressed IRI generation port: `ContentAddressedIriGenerator.generateIri(graph)` over `ReadableGraph`, returning a deterministic `urn:cid:` derived from the graph's triples — every term in full, datatype and language tag included — for a graph describing exactly one IRI subject. Unlike the other port families it carries its own implementation, `ContentAddressedIriGeneratorCbor` — there is no backend to swap. No dependency on the dataset ports. |
