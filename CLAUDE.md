...
| `rdf-dataset` | `io.kogn.rdf:rdf-dataset` | Backend-agnostic dataset content ports: `GraphStore`, `SparqlQuery` (read), `SparqlUpdate` (write), `DatasetExport` (serialize the dataset or one named graph to a stream, in an `RdfFormat`; non-transactional, see ADR-0013), `DatasetTransactor`, `DatasetTx`. Nothing here presumes the library hosts the store. |
...
| `rdf-cid` | `io.kogn.rdf:rdf-cid` | Content-addressed IRI generation port: `ContentAddressedIriGenerator.generateIri(graph)` returns a deterministic `urn:cid:` derived from the graph's triples (URDNA2015 canonicalization, blank node skolemization, SHA3-256 over a canonical, length-prefixed S-expression serialization). Every term goes into the digest in full, datatype and language tag (case-normalized) included; a graph without exactly one IRI subject, or with triples that subject does not reach, is rejected (`IllegalArgumentException`) — see ADR-0014. Port and its sole implementation `ContentAddressedIriGeneratorSexpr` share this leaf module, as in `rdf-shacl`. **Depends on `rdf-terms` alone among our modules** — no rdf4j — and is the only backend-free module in this repository with a third-party dependency footprint of any size: `io.setl:rdf-urdna`, `com.apicatalog:titanium-json-ld`, Commons Codec; the RDF4J adapter modules pull considerably more. |
