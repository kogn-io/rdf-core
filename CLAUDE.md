...
| `rdf-cid` | `io.kogn.rdf:rdf-cid` | Content-addressed IRI generation port: `ContentAddressedIriGenerator.generateIri(graph)` returns a deterministic `urn:cid:` derived from the graph's triples (URDNA2015 canonicalization, blank node skolemization, Blake2b-256 over a canonical serialization). Every term goes into the digest in full, datatype and language tag included; a graph without exactly one IRI subject, or with triples that subject does not reach, is rejected (`IllegalArgumentException`) — see ADR-0014. Port and its sole implementation `ContentAddressedIriGeneratorCbor` share this leaf module, as in `rdf-shacl`. The `Cbor`/`cbor` names are a misnomer inherited from the origin — the hashed form is an S-expression. **Depends only on `rdf-terms`** — no rdf4j. |
